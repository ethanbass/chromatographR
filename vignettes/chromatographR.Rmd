...
Experimental data can suffer from a number of non-informative artifacts, such as noise, shifting baselines, or missing values. In addition, the data produced by the instrument may have longer wavelength or time ranges (or higher resolution) than we need. Since HPLC-DAD data are fairly smooth, these types of issues can usually be corrected fairly easily. For example, smoothing can reduce noise in the spectral direction while baseline subtraction can help correct a shifting baseline. Selection of and interpolation of wavelengths and retention times can be used to reduce the dimensionality of the data, facilitating comparison across samples and reducing the computation load of downstream analyses. All of these functions are available through the `preprocess` function. Smoothing and baseline subtraction are enabled by default. Additionally, alternative times (`dim1`) and/or wavelengths (`dim2`) can be provided to interpolate data along one or both axes. This can significantly reduce the computational time and memory requirements in later steps.
...
You can see that, in this case, the alignment is considerably improved after warping. You can also do a global alignment on multiple wavelengths at the same time, but it will not always improve the results. While the alignment still isn't perfect here, it is probably good enough to align our peaks and collate assemble them in the peak table, which is our main goal.

# Warped goldenrod root chromatograms.

A list of four pre-processed and warped goldenrod root chromatograms
derived from the raw data stored in
[`Sa`](https://ethanbass.github.io/chromatographR/reference/Sa.md).

## Usage

``` r
data(Sa_warp)
```

## Format

A list of four pre-processed and warped matrices (434 times x 60
wavelengths).

## See also

Other data objects:
[`Sa`](https://ethanbass.github.io/chromatographR/reference/Sa.md),
[`Sa_pr`](https://ethanbass.github.io/chromatographR/reference/Sa_pr.md),
[`pk_tab`](https://ethanbass.github.io/chromatographR/reference/pk_tab.md)
